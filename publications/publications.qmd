...
bib  <- yaml::read_yaml("publications.yml")
bibtext  <- ReadBib("publications.bib", check = TRUE)
keys <- names(bibtext) 

#reorder the extra data so it's in the same order as bib  

rows <- tibble(key = keys,
               entry = lapply(keys, function(k) bibtext[k]),
               title = sapply(bib, `[[`, "title"),
               authors_card = map_chr(entry, fmt_authors_card),
               year = sapply(bib, get_date),
               venue = sapply(bib, function(x){x$journal %||% x$booktitle %||%x$publisher %||% ""}),
               abstract = sapply(bib, function(x){x$abstract %||% ""}),
               bibtex = map_chr(entry, get_bibtex),
               doi = sapply(bib, function(x){x$doi %||% ""}),
               url_fallback = sapply(bib, function(x){x$url %||% ""}),
               preprint_raw  = sapply(bib, function(x){v <- x$preprint; if(is.null(v) || length(v)==0) NA_character_ else as.character(v)}),
               materials_raw  = sapply(bib, function(x){v <- x$materials; if(is.null(v) || length(v)==0) NA_character_ else as.character(v)}),
  ...
  mutate(
    primary = tolower(as.character(sapply(bib, `[[`, "primary")))
